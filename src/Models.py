...
class SelfAttAggregate(nn.Module):
    ...
    def forward(self, hiddens):
        ...
        # Aggregate using max pooling and average pooling along the temporal dimension
...
class CRASH(nn.Module):
    def __init__(self, x_dim, h_dim, z_dim, n_layers=1, n_obj=19, n_frames=100, fps=20.0, with_saa=True):
        ...
        self.x_dim = x_dim # 4096
        self.h_dim = h_dim # 512
        self.z_dim = z_dim
        self.n_layers = n_layers
        self.n_obj = n_obj
    ...
    def forward(self, x, y, toa, hidden_in=None, nbatch=80, testing=False):
        ...
        for t in range(x.size(1)):
            ...
            img_fft = self.phi_x3(img_tmp) # 10,1,16
            # 为了做object attention
            obj_embed = x_t[:, 1:, :]
